...
from utils.scripts import get_playlists
from threadedreturn import ThreadWithReturnValue
...
def search():
    ...
    if request.method == "POST":
        ...
        return render_template(
            "search.html", results=clean_returned, playlists=playlists
        )
    else:
        return render_template("search.html", results=None)


@search_bp.route("/player", methods=["GET"])
async def player():
    songs_list = requests.get("https://musicbackend.lunes.host/list_songs").json()
    songs_info = random.choices(songs_list, k=5)
    if "email" in session:
        playlists = get_playlists(session["email"])
    else:
        playlists = None

    return render_template("player.html", results=songs_info, playlists=playlists)
