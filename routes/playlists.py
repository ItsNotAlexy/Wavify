...
from utils.scripts import email_to_id, get_playlists, fetch_all_song_info
from youtube_search import YoutubeSearch
from utils.spotify import get_all_song_names
...
def playlist():
    ...
    playlists = db.execute(
        "SELECT * FROM playlists WHERE owner_id = ?", (user_id[0],)
    ).fetchall()
...
def add_song():
    ...
    res = db.execute(
        ...
        (email_to_id(session["email"])[0], pl_id),
    ).fetchone()
    if res is not None:
        old_songs = res[2]
...
async def play_playlist(pl_id: str):
    ...
    owner_name = db.execute(
        "SELECT display_name from users where id = ?", (res[1],)
    ).fetchone()
